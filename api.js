...
async function register(phone, password) {
  return apiRequest('/api/auth/register', 'POST', { phone, password });
}
...
// ── Export ────────────────────────────────────
window.API = {
  register, login, verifyToken, getUserInfo,
  generateApiKey,
  sendReaction, downloadMedia, enhanceHD,
  getStatus,
  admin: {
    login: adminLogin, getStats: adminGetStats, getUsers: adminGetUsers,
    setUserStatus: adminSetUserStatus, banUser: adminBanUser, setLimit: adminSetLimit,
    saveSetting: adminSaveSetting, getBots: adminGetBots,
    connectBot: adminConnectBot, deleteBot: adminDeleteBot, toggleBot: adminToggleBot
  }
};
